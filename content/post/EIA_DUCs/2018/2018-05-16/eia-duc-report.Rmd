...
* Permian Basin ended April with **~3,086 wells waiting on completion.**   
* For the past eight months, the **DUC to Completion Ratio** has remained steady at **~6.5:1.**
...
P1=ggplot(subset(AllBasins, Date<="2016-01-02"), aes(Date, DUC, group=Basin, color=Basin, frame=Date))+geom_line(size=2)+facet_wrap(~Basin, scales="free")+
  ...
  '2014-01-01','2018-05-01')))+ theme(strip.text.x = element_text(size = 12))+scale_y_continuous(labels=comma)+
  scale_x_date(breaks= pretty_breaks(), limits = as.Date(c('2014-01-01','2018-05-01')))+ 
...
P3=ggplot()+geom_area(data=AllBasins, aes(Date,Drilled,fill="Drilled" ), stat='identity', alpha=.75,show.legend =T)+theme_minimal()+
  ...
  scale_x_date(breaks= pretty_breaks(), limits = as.Date(c('2014-01-01','2018-05-01')))+ theme(strip.text.x = element_text(size = 12))+scale_y_continuous(labels=comma, breaks= pretty_breaks())+
