...
leaflet(egn,width="100%",height = 550) %>%
  ...
  addTiles(group = "Esri.WorldTopoMap")%>%
  addProviderTiles(providers$Esri.WorldTopoMap)%>%
  addPolygons(fillOpacity = .5,fillColor = 'red',stroke=F,group = "Energen Acreage")%>%
  addPolygons(data=cxo_map,fillOpacity = .5,fillColor = 'yellow',stroke=F,group = "Concho Acreage")%>%
  addPolylines(data=lines,weight=1,color='black')%>%
  addCircleMarkers(data=wells,~Surface.Hole.Longitude..WGS84.,~Surface.Hole.Latitude..WGS84.,radius=5, stroke = F,color=c("blue",'green'),fillOpacity=.75, group = "Energen Cline Wells",   popup=paste("Company:", wells$Operator.Alias, "<br>",
...
<img style="margin:0px auto;display:block" src="/img/1q18/egn_ranger.png"/>  
<br>
