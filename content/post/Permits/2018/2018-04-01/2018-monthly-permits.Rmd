...
URL<-url_parse(URL)
URL$parameter= 'state_province=New%20Mexico&min_approved_date=2018-04-01&pagesize=100000'
...
#Be sure to Update Month
URL<-url_parse(URL)
URL$parameter= 'state_province=Texas&min_approved_date=2018-04-01&pagesize=100000'
...
<h4 style="text-align: center;" markdown="1">**April Permitting Activity**</h4>
```{r, echo=F}
map1
```
<br>
...
1. Use mouse to scroll in or click on circle with numbers  
    i. Continue to scroll or zoom in until numbers are lower than 10 
2. Selected permits will appear when clicked or touched (mobile or ipad device)
    i. Circles on bottom right are more accessible than cirlcles positioned above (working on a solution)
...
ggplot(Top20f, aes(x=Top20,fill=Top20))+geom_bar()+scale_fill_manual(values=col)+coord_flip()+hrbrthemes::theme_ipsum()+theme(legend.position = 'none')+ggtitle('Top 20 Operators Permitting Activity in April', subtitle = 'Sourced from DrillingInfo')
