...
* Added capability to filter to company permits and acreage.
* Export table now includes surface longitude and suface latitude.
<br>
...
* 2^nd^ map identifies **permitted well pads** using clustering.
...
```{r}
competitors<-readOGR(".","competitors", verbose=F)

competitors<-st_as_sf(competitors)

competitors<- left_join(competitors,alias, by=c("Company"="operator_alias"))

company_filter<- as.data.frame(unique(unique(tx_nm_permits$Alias)))
colnames(company_filter)[1]<-"Alias"

competitors<- left_join(company_filter,competitors)%>% filter(!is.na(Link))

competitors<-st_as_sf(competitors)

competitors<- as(competitors,"Spatial")

competitors$Reset_Filter='RESET'

```

```{r}
competitors_sd<- SharedData$new(competitors)
tx_nm_permits_sd<- SharedData$new(tx_nm_permits)
 
#Added this to allow sharedDate to work ()

competitors_sd_df <- SharedData$new(competitors@data, group = competitors_sd$groupName())

```

<h4 style="text-align: center;" markdown="1">**May Permitting Activity**</h4>
```{r}
filter_select('Alias','Company Permits',tx_nm_permits_sd,~Alias)
filter_select('Alias','Company Acreage',competitors_sd_df,~Alias)

leaflet(width="100%",height = 550) %>%
  # Base groups
  addTiles(group = "Esri.WorldTopoMap")%>%
  addProviderTiles(providers$Esri.WorldTopoMap)%>%
  addPolygons(data=cxo_leases,fillColor='yellow',fillOpacity = .4,stroke=NA)%>%
  addPolygons(data=competitors_sd,fillColor='grey',fillOpacity = .3,stroke=NA,group="Competitor Acreage")%>%
  addCircleMarkers(data=tx_nm_permits_sd,~surface_longitude,radius=10, ~surface_latitude, stroke = F,color=~pal(Top20),fillOpacity=.8, group = "Permitting Activity", 
  ...
  setView(lng=-102.79, lat=31.89, zoom=7)%>%
  # Overlay groups
  addLayersControl(
    overlayGroups = c("Permitting Activity","Competitor Acreage"),
...
leaflet(width="100%",height = 550) %>%
  ...
  addProviderTiles(providers$Esri.WorldTopoMap)%>%
  addPolygons(data=cxo_leases,fillColor='yellow',fillOpacity = .4,stroke=NA)%>%
  addCircleMarkers(data=tx_nm_permits_sd,clusterOptions= markerClusterOptions(freezeAtZoom=13, spiderfyDistanceMultiplier=3),~surface_longitude,radius=10, ~surface_latitude, stroke = F,color=~pal(Top20),fillOpacity=.6, group = "Permitting Activity", 
  popup=paste("Company:", tx_nm_permits$operator_alias, "<br>",
              "Well Name:", tx_nm_permits$lease_name," ",tx_nm_permits$well_number," ",tx_nm_permits$drill_type,"<br>",
              "Well Type:", tx_nm_permits$well_type,"<br>",
              "Approved Date:",tx_nm_permits$approved_date,"<br>",
  ...
  # Overlay groups
  addLayersControl(
    overlayGroups = c("Permitting Activity"),
    options = layersControlOptions(collapsed = FALSE),position='topleft')%>%
  addLegend('topright',pal=pal,values=tx_nm_permits$Top20,title="Company",opacity=1)%>%
  addFullscreenControl( )%>%suspendScroll()%>% addScaleBar(position="topleft")
```  
...
```{r,echo=F}
table<-DT::datatable(tx_nm_permits[,c(1:4,23,9,6,17,18)],class = 'cell-border stripe',filter='top',extensions='Buttons',
