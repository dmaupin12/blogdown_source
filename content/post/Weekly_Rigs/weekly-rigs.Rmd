...
RankChart<- chart%>%filter(Basin=="Permian")%>% group_by(county)%>% mutate(TotalRig= sum(rigcnt))
...
plot<-ggplot(chart[,c(3,9,2)], aes(PublishDate,rigcnt, group=county))+geom_line(color='grey', size=1, alpha=.5)+
  ...
  ggtitle('NM and TX Change in Rig Count By County', subtitle = 'Top 10 Permian Counties Highlighted')+
  labs(x="Publish Date",y="Rig Count")
